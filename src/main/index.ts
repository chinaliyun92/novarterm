...
import { clearTriggerIPCHandlers, registerTriggerIPCHandlers } from './ipc/trigger'
import { clearSSHIPCHandlers, registerSSHIPCHandlers, shutdownSSHService } from './ipc/ssh'
import {
  clearTerminalIPCHandlers,
  registerTerminalIPCHandlers,
  requestRendererTerminalPrepareForQuit,
} from './ipc/terminal'
...
import { normalizeMainUpdateLanguage } from './update-locale'
import { UPDATE_IPC_EVENTS } from '../shared/ipc/channels'
...
const TERMINAL_QUIT_FLUSH_TIMEOUT_MS = 900
const UI_LANGUAGE_SETTING_KEY = 'ui.language'
...
let quitAfterTerminalFlush = false
let terminalQuitFlushPromise: Promise<void> | null = null
let shutdownComplete = false
let shutdownPromise: Promise<void> | null = null
let resolveUpdateLanguage = (): string => normalizeMainUpdateLanguage(app.getLocale())
...
function shouldBlockWindowReloadShortcut(input: Electron.Input): boolean {
  ...
  // Block Cmd/Ctrl + R and Cmd/Ctrl + Shift + R in all environments.
  return key === 'r' && (input.meta || input.control)
}
...
function applyApplicationMenu(): void {
    ...
    submenu: [
      {
        ...
        click: () => {
          void shell.openExternal(UPDATE_RELEASES_PAGE_URL).catch((error: unknown) => {
            const message = error instanceof Error ? error.message : String(error)
            logger.warn('app', `failed to open release page: ${message}`)
          })
        },
      },
    ],
  ...
  logger.info('app', 'applied application menu (mac: App + Edit + Help, others: Edit + Help)')
}
...
function createUpdateContext(): UpdateIPCContext {
  return {
    ...
    logger,
    getLanguage: () => resolveUpdateLanguage(),
  }
}

function broadcastNativeUpdateState(state: import('../shared/types/update').NativeUpdateState): void {
  for (const win of BrowserWindow.getAllWindows()) {
    if (!win.isDestroyed()) {
      win.webContents.send(UPDATE_IPC_EVENTS.stateChanged, state)
    }
  }
}

function buildUpdateLanguageResolver(
  settingsRepository: Pick<SettingsRepository, 'findById'>,
): () => string {
  return () => {
    const languageSetting = settingsRepository.findById(UI_LANGUAGE_SETTING_KEY)?.value ?? null
    if (languageSetting) {
      return normalizeMainUpdateLanguage(languageSetting)
    }
    return normalizeMainUpdateLanguage(app.getLocale())
  }
}
...
async function runAutoUpdateCheck(): Promise<void> {
  ...
  try {
    if (isNativeUpdaterAvailable()) {
      ...
      const alreadyHandled = state.phase === 'downloading' || state.phase === 'downloaded'
      const hasUpdate = checkResult?.isUpdateAvailable ?? false
      if (hasUpdate && !alreadyHandled) {
        await downloadUpdateWithInstaller().catch((error: unknown) => {
          const message = error instanceof Error ? error.message : String(error)
          logger.warn('update', `auto update download failed: ${message}`)
        })
      }
      return
    }
    ...
    const promptResult = await promptForUpdateAndMaybeOpen(
      targetWindow,
      {
      ...
      createUpdateContext().getLanguage?.() ?? null,
    )
...
function clampWindowState(rawState: PersistedWindowState): PersistedWindowState {
  ...
  const candidateBounds = {
    x: rawState.x ?? fallbackDisplay.workArea.x,
    y: rawState.y ?? fallbackDisplay.workArea.y,
    width: rawState.width,
    height: rawState.height,
  }
  const matchedDisplay = screen.getDisplayMatching(candidateBounds)
  const isOnKnownDisplay = displays.some((display) => display.id === matchedDisplay.id)
  const display = isOnKnownDisplay ? matchedDisplay : fallbackDisplay
  ...
  // If saved coordinates are far outside the chosen work area, snap back to a visible origin.
  const rawX = typeof rawState.x === 'number' ? rawState.x : null
  const rawY = typeof rawState.y === 'number' ? rawState.y : null
  const visiblyOverlaps =
    rawX !== null &&
    rawY !== null &&
    rawX < workArea.x + workArea.width &&
    rawX + width > workArea.x &&
    rawY < workArea.y + workArea.height &&
    rawY + height > workArea.y

  const x =
    rawX === null
      ? undefined
      : visiblyOverlaps
        ? Math.max(workArea.x, Math.min(rawX, maxX))
        : workArea.x + Math.max(0, Math.floor((workArea.width - width) / 2))
  const y =
    rawY === null
      ? undefined
      : visiblyOverlaps
        ? Math.max(workArea.y, Math.min(rawY, maxY))
        : workArea.y + Math.max(0, Math.floor((workArea.height - height) / 5))
...
function registerIpcHandlers(): void {
  ...
  registerTriggerIPCHandlers(ipcMain, repositories)
  logger.info('ipc', 'registered trigger IPC handlers')
  resolveUpdateLanguage = buildUpdateLanguageResolver(repositories.settings)
  if (app.isPackaged) {
    setupAutoUpdater({
      logger,
      releasesPageUrl: UPDATE_RELEASES_PAGE_URL,
      currentVersion: app.getVersion(),
      broadcastUpdateState: broadcastNativeUpdateState,
    })
  }
...
function createMainWindow(): void {
  ...
  mainWindow.webContents.on('before-input-event', (event, input) => {
    if (!shouldBlockWindowReloadShortcut(input)) {
      return
    }
    ...
    logger.info(
      'window',
      `blocked reload shortcut key=${input.key} meta=${String(input.meta)} control=${String(input.control)} shift=${String(input.shift)}`,
    )
  })
  ...
  mainWindow.once('ready-to-show', () => {
    ...
    logger.info('window', 'ready-to-show -> focus')
...
app.whenReady().then(() => {
  ...
  logger.error('app', `startup failed: ${details}`)
  try {
    dialog.showErrorBox(
      'NovarTerm failed to start',
      [
        'Native modules may need to be rebuilt for Electron after installing dependencies.',
        '',
        'Try: pnpm rebuild:native',
        '',
        details.slice(0, 1200),
      ].join('\n'),
    )
  } catch {
    // ignore dialog failures during broken startup
  }
  app.quit()
})
...
app.on('before-quit', (event) => {
  ...
  event.preventDefault()
  if (terminalQuitFlushPromise) {
    logger.info('terminal-quit', 'quit flush already in progress')
    return
  }

  logger.info('terminal-quit', `requesting renderer quit flush timeoutMs=${TERMINAL_QUIT_FLUSH_TIMEOUT_MS}`)
  terminalQuitFlushPromise = requestRendererTerminalPrepareForQuit(
    targetWindow.webContents,
    TERMINAL_QUIT_FLUSH_TIMEOUT_MS,
    logger,
  )
    .then((completed) => {
      logger.info('terminal-quit', `renderer quit flush completed=${String(completed)}`)
    })
    .catch((error: unknown) => {
      const message = error instanceof Error ? error.message : String(error)
      logger.warn('terminal-quit', `renderer quit flush failed: ${message}`)
    })
    .finally(() => {
      terminalQuitFlushPromise = null
      if (quitAfterTerminalFlush) {
        return
      }
      quitAfterTerminalFlush = true
      app.quit()
    })
})
...
app.on('will-quit', (event) => {
  ...
  logger.info('app', 'will-quit -> starting shutdown')
  shutdownPromise = (async () => {
    try {
      await shutdownSSHService()
    } catch (error: unknown) {
      const message = error instanceof Error ? error.message : String(error)
      logger.warn('app', `ssh shutdown failed: ${message}`)
    }
