...
let terminalPrepareForQuitSequence = 0;
const pendingTerminalPrepareForQuitResolvers = new Map<string, () => void>();
...
export function registerTerminalIPCHandlers(
  ...
  ipcMain.handle(
    ...
    async (_event: IpcMainInvokeEvent, request: TerminalLoadPersistedRequest) =>
      toResult(async () => {
        ...
        options.logger?.info(
          "terminal-replay",
          `loadPersisted request session=${request.sessionId} maxBytes=${request.maxBytes ?? "default"}`,
        );
        const data =
        ...
        options.logger?.info(
          "terminal-replay",
          `loadPersisted result session=${request.sessionId} transcriptChars=${data.transcript.length} cwd=${data.cwd ?? "(none)"} elapsedMs=${Math.max(0, Date.now() - startedAt)}`,
        );
  ...
  ipcMain.handle(
    TERMINAL_IPC_CHANNELS.saveCwd,
    async (_event: IpcMainInvokeEvent, request: TerminalSaveCwdRequest) =>
      toResult(async () => {
        localTerminalPersistenceService?.saveCwd({
          sessionId: request.sessionId,
          cwd: request.cwd,
        });
        ...
        return {
          saved: true,
        } satisfies TerminalSaveCwdResponse;
      }),
  );

  ipcMain.handle(
    TERMINAL_IPC_CHANNELS.prepareForQuitComplete,
    async (_event: IpcMainInvokeEvent, request: TerminalPrepareForQuitCompleteRequest) =>
      toResult(async () => {
        const token = typeof request?.token === "string" ? request.token.trim() : "";
        if (token) {
          const resolve = pendingTerminalPrepareForQuitResolvers.get(token);
          if (resolve) {
            pendingTerminalPrepareForQuitResolvers.delete(token);
            resolve();
          }
        }
        ...
        return {
          acknowledged: true,
        } satisfies TerminalPrepareForQuitCompleteResponse;
      }),
  );

  ipcMain.handle(
    TERMINAL_IPC_CHANNELS.cleanPersistedLogs,
    async (_event: IpcMainInvokeEvent, request: TerminalCleanPersistedLogsRequest) =>
      toResult(async () => {
        const keepSessionIds = Array.isArray(request?.keepSessionIds) ? request.keepSessionIds : [];
        const removedCount = localTerminalPersistenceService
          ? await localTerminalPersistenceService.cleanupUnusedSessionLogs(keepSessionIds)
          : 0;
        return {
          removedCount,
        } satisfies TerminalCleanPersistedLogsResponse;
      }),
  );
...
export function clearTerminalIPCHandlers(ipcMain: IpcMainLike): void {
  ...
  pendingTerminalPrepareForQuitResolvers.clear();
  localTerminalPersistenceService = null;
}

export async function requestRendererTerminalPrepareForQuit(
  sender: WebContents,
  timeoutMs: number,
  logger?: AppLogger,
): Promise<boolean> {
  ...
  const normalizedTimeoutMs = Number.isFinite(timeoutMs)
    ...
    : 800;
  const token = `quit-flush-${Date.now()}-${++terminalPrepareForQuitSequence}`;
  ...
  return new Promise<boolean>((resolve) => {
    ...
    pendingTerminalPrepareForQuitResolvers.set(token, () => settle(true));
    timeoutHandle = setTimeout(() => {
      logger?.warn(
        "terminal-quit",
        `renderer quit flush timed out after ${normalizedTimeoutMs}ms token=${token}`,
      );
      settle(false);
    }, normalizedTimeoutMs);

    try {
      const payload: TerminalPrepareForQuitEvent = {
        token,
        timeoutMs: normalizedTimeoutMs,
      };
      sender.send(TERMINAL_IPC_EVENTS.prepareForQuit, payload);
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      logger?.warn("terminal-quit", `failed to send renderer quit flush request: ${message}`);
      settle(false);
    }
  });
}
...
function sendToSender(
  sender: WebContents,
  channel: typeof TERMINAL_IPC_EVENTS.prepareForQuit,
  payload: TerminalPrepareForQuitEvent,
): void;
function sendToSender(
  sender: WebContents,
  channel: string,
  payload: TerminalDataEvent | TerminalExitEvent | TerminalErrorEvent | TerminalPrepareForQuitEvent,
): void {
  if (sender.isDestroyed()) {
    return;
  }
...
function createPersistenceService(
  ...
  const service = new LocalTerminalPersistenceService(baseDir, options.settingsRepository, options.logger);
