...
import { getUpdateDialogCopy } from '../update-locale'
...
export async function promptForUpdateAndMaybeOpen(
  ...
  const copy = getUpdateDialogCopy(language)
  const options: MessageBoxOptions = {
    ...
    message: copy.updateAvailableMessage(request.latestVersion),
    detail: [
      `${copy.currentVersionLabel}: ${request.currentVersion}`,
      `${copy.latestVersionLabel}: ${request.latestVersion}`,
      request.latestTag ? `${copy.releaseTagLabel}: ${request.latestTag}` : '',
      '',
      copy.openDownloadPageHint,
    ]
      .filter(Boolean)
      .join('\n'),
  }
...
export function registerUpdateIPCHandlers(
  ...
  ipcMain.handle(UPDATE_IPC_CHANNELS.getState, async () =>
    toResult<NativeUpdateState>(() => {
      if (!useNative) {
        throw new UpdateIPCError('unknown_error', 'Native updater is unavailable')
      }
      return getNativeUpdateState()
    }),
  )

  ipcMain.handle(UPDATE_IPC_CHANNELS.download, async () =>
    toResult<UpdateDownloadResponse>(async () => {
      if (!useNative) {
        throw new UpdateIPCError('unknown_error', 'Native updater is unavailable')
      }
      void downloadUpdateWithInstaller().catch(() => {})
      return {
        started: true,
      } satisfies UpdateDownloadResponse
    }),
  )

  ipcMain.handle(UPDATE_IPC_CHANNELS.restartToInstall, async () =>
    toResult<UpdateRestartResponse>(() => {
      if (!useNative) {
        throw new UpdateIPCError('unknown_error', 'Native updater is unavailable')
      }
      quitAndInstallUpdate()
      return {
        accepted: true,
      } satisfies UpdateRestartResponse
    }),
  )
  ...
  ipcMain.handle(
    ...
    async (event: IpcMainInvokeEvent, request: UpdatePromptRequest) =>
      toResult(async () => {
        ...
        return promptForUpdateAndMaybeOpen(
          parentWindow,
          request,
          context.releasesPageUrl,
