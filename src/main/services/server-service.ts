...
export class ServerService {
  ...
  public async updateServer(serverId: number, input: UpdateServerInput): Promise<Server> {
    ...
    const shouldDropSessions = this.isConnectionConfigChange(existing, input);
    if (shouldDropSessions) {
      await this.disconnectSessionsForServer(serverId);
    }
  ...
  private async disconnectSessionsForServer(serverId: number): Promise<void> {
    const sessionIds = Array.from(this.sessionServerBindings.entries())
      .filter(([, boundServerId]) => boundServerId === serverId)
      .map(([sessionId]) => sessionId);

    for (const sessionId of sessionIds) {
      try {
        await this.sshService.disconnect(sessionId);
      } catch {
        // Best-effort: continue cleaning bindings even if one disconnect fails.
      }
      this.sessionServerBindings.delete(sessionId);
    }
  }

  private isConnectionConfigChange(existing: Server, input: UpdateServerInput): boolean {
    if (input.host !== undefined && input.host !== existing.host) {
      return true;
    }
    ...
    if (input.authType !== undefined && input.authType !== existing.authType) {
      return true;
    }
    if (input.password !== undefined && input.password !== existing.password) {
      return true;
    }
    if (input.privateKeyPath !== undefined && input.privateKeyPath !== existing.privateKeyPath) {
      return true;
    }
    if (input.passphrase !== undefined && input.passphrase !== existing.passphrase) {
      return true;
    }
    return false;
  }
