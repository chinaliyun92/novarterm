...
export class SSHConnectionManager {
  ...
  private connectInFlight: Promise<SSHConnectionSnapshot> | null = null;
  ...
  public async connect(config: SSHConnectionConfig): Promise<SSHConnectionSnapshot> {
    ...
    this.connectInFlight = this.performConnect(config).finally(() => {
      this.connectInFlight = null;
    });
    return this.connectInFlight;
  }

  private async performConnect(config: SSHConnectionConfig): Promise<SSHConnectionSnapshot> {
    if (!config.host) {
    ...
    await new Promise<void>((resolve) => {
      ...
      setTimeout(() => {
        try {
          ...
          // Ignore destroy errors during forced shutdown.
        }
        finish();
      }, 200);
    });
  ...
  private async connectInternal(
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      try {
        ...
        // Ignore destroy errors on failed connect attempts.
      }
    ...
    if (this.client !== client || this.userInitiatedDisconnect) {
      try {
        client.end();
      } catch {
        // Ignore end errors when connect was superseded.
      }
      if (this.client === client) {
        this.client = null;
      }
      throw new SSHServiceError("connection_error", "SSH connect was superseded");
    }
