...
interface SSHSessionContext {
  ...
  remoteCwd?: string;
  terminalCallbacks?: TerminalEventCallbacks;
  connectInFlight: Promise<SSHConnectionSnapshot> | null;
  openTerminalInFlight: Promise<TerminalOpenResult> | null;
}
...
export class SSHService {
  ...
  public async connect(
    ...
    if (context.connectInFlight) {
      return context.connectInFlight;
    }

    context.connectInFlight = this.performConnect(sessionId, config).finally(() => {
      context.connectInFlight = null;
    });
    return context.connectInFlight;
  }
  ...
  public async disconnect(sessionId: string): Promise<SSHConnectionSnapshot> {
    ...
    if (!context) {
      return {
        state: "disconnected",
      } as SSHConnectionSnapshot;
    }
    await context.sftpService.disconnect();
    const snapshot = await context.connectionManager.disconnect();
    context.remoteCwd = undefined;
    return snapshot;
  }
  ...
  public async removeSession(sessionId: string): Promise<void> {
    ...
    await context.sftpService.disconnect();
    await context.connectionManager.disconnect();
  ...
  public async shutdown(): Promise<void> {
    ...
    await Promise.allSettled(sessionIds.map((sessionId) => this.removeSession(sessionId)));
  }
  ...
  public async openTerminal(
    ...
    const context = this.getOrCreateContext(sessionId);
    context.terminalCallbacks = callbacks;

    if (context.openTerminalInFlight) {
      return context.openTerminalInFlight;
    }

    context.openTerminalInFlight = this.performOpenTerminal(sessionId, options, callbacks).finally(() => {
      context.openTerminalInFlight = null;
    });
    return context.openTerminalInFlight;
  }

  private async performOpenTerminal(
    sessionId: string,
    options: TerminalOpenOptions,
    callbacks: TerminalEventCallbacks,
  ): Promise<TerminalOpenResult> {
    ...
    terminalContext = this.createTerminalContext(
      sessionId,
      channel,
      callbacks,
      () => {
  ...
  private createTerminalContext(
    ...
    const resolveCallbacks = (): TerminalEventCallbacks => {
      return this.sessions.get(sessionId)?.terminalCallbacks ?? callbacks;
    };
    ...
    const onStdoutData = (chunk: Buffer | string) => {
      const data = toChunkString(chunk);
      const nextCwd = extractLatestCwdFromAnsiChunk(data);
      if (nextCwd) {
        onCwdUpdate?.(nextCwd);
      }

      resolveCallbacks().onData({
        sessionId,
        stream: "stdout",
        data,
      });
...
function prepareLocalShellBootstrapEnv(
  ...
  if (shellName === "bash") {
    const novartermHistoryCommand = "history -n";
    ...
    const hasHistoryReload = /\bhistory\s+-n\b/.test(existingPromptCommand);
    const hasPromptMarker = existingPromptCommand.includes("]133;D;");
    const promptCommandSegments = [
      ...(hasHistoryReload ? [] : [novartermHistoryCommand]),
      ...(hasPromptMarker ? [] : [novartermPromptCommand]),
      ...(existingPromptCommand ? [existingPromptCommand] : []),
    ];
    const mergedPromptCommand = promptCommandSegments.join(";");
    return {
      env: {
        ...baseEnv,
  ...
  try {
    ...
    writeScript(
      ...
      [
        ...
        '  source "$HOME/.zshenv"',
        "fi",
        'if [ -n "${NOVARTERM_ORIGINAL_ZDOTDIR:-}" ]; then',
        '  export ZDOTDIR="$NOVARTERM_ORIGINAL_ZDOTDIR"',
        "else",
        "  unset ZDOTDIR",
        "fi",
        "unset NOVARTERM_ORIGINAL_ZDOTDIR",
    ...
    writeScript(
      ...
      [
        ...
        "fi",
        "if (( ${+functions[__novarterm_history_precmd]} == 0 )); then",
        "  function __novarterm_history_precmd() {",
        '    if [ -n "${HISTFILE:-}" ] && [ -f "$HISTFILE" ]; then',
        '      fc -RI "$HISTFILE" 2>/dev/null || true',
        "    fi",
        "  }",
        "fi",
        "if (( ${precmd_functions[(I)__novarterm_history_precmd]} == 0 )); then",
        "  precmd_functions=(__novarterm_history_precmd ${precmd_functions[@]})",
        "fi",
    ...
    return {
      env: {
        ...
        NOVARTERM_ORIGINAL_ZDOTDIR: typeof baseEnv.ZDOTDIR === "string" ? baseEnv.ZDOTDIR : "",
      },
      cleanup: () => {
        try {
