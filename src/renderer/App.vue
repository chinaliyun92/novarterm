...
import { useTerminalTriggers } from './src/composables/useTerminalTriggers'
...
const settingsDialogInitialTab = ref<SettingsTab | undefined>(undefined)
...
function createTabFromShortcut(): void {
  const currentSessionId = activeSessionId.value
  const currentSession = activeWorkspaceSession.value
  const currentCwd = terminalCwd.value?.trim() ?? ''
  const shouldInheritLocalCwd =
    currentSession?.kind === 'local' &&
    !isSessionInSshShell(currentSessionId) &&
    Boolean(currentCwd)

  const created = workspace.createSession({ activate: true, kind: 'local' })
  if (!created || !shouldInheritLocalCwd) {
    return
  }

  sessionPathSync.hintCwd(created.id, currentCwd)
}
...
function openSettingsDialog(tab?: SettingsTab): void {
...
async function shouldShowShortcutsGuideOnFirstLaunch(): Promise<boolean> {
  const settingsApi = getElectronSettingsApi()
  if (settingsApi) {
    try {
      const result = await settingsApi.get(FIRST_LAUNCH_SHORTCUTS_GUIDE_KEY)
      if (!result.ok) {
        throw new Error(result.error.message)
      }

      if (result.data.setting?.value === '1') {
        return false
      }

      const writeResult = await settingsApi.set(FIRST_LAUNCH_SHORTCUTS_GUIDE_KEY, '1')
      if (!writeResult.ok) {
        throw new Error(writeResult.error.message)
      }

      try {
        window.localStorage.removeItem(FIRST_LAUNCH_SHORTCUTS_GUIDE_KEY)
      } catch {
...
onMounted(() => {
  void checkIpc()
  void Promise.allSettled([triggerState.ensureLoaded(), serverState.ensureLoaded()]).then((results) => {
  ...
  void shouldShowShortcutsGuideOnFirstLaunch().then((shouldOpen) => {
    if (shouldOpen) {
...
watch(
  ...
  (servers) => {
    if (!serverState.initialized.value || !triggerState.loaded.value) {
      return
    }
    triggerState.syncHiddenServerRules(servers)
  },
  { deep: true },
)
