...
import { normalizeRemotePath, toErrorMessage, getRemoteParentPath, getRemoteBasename } from '../../types/file-browser'
...
async function remotePathExists(remotePath: string): Promise<boolean> {
  const sessionId = sessionIdValue.value
  if (!sessionId) {
    return false
  }

  const parentPath = getRemoteParentPath(remotePath) ?? '/'
  const entryName = getRemoteBasename(remotePath)
  if (!entryName) {
    return false
  }
  ...
  const listApi = window.electronAPI?.ssh?.sftp?.list
  ...
  return result.data.some((entry) => entry.name === entryName)
}
...
async function handleSaveAs(): Promise<void> {
  ...
  if (await remotePathExists(targetPath)) {
    const confirmed = window.confirm(
      t('file.inspector.confirm.saveAsOverwrite', { path: targetPath }),
    )
    if (!confirmed) {
      return
    }
