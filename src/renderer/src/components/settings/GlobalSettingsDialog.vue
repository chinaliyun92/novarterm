...
const SETTINGS_ACTIVE_TAB_STORAGE_KEY = 'iterm.settings.active-tab'
...
const primaryShortcutLabel = (() => {
  ...
  const userAgentPlatform =
    ...
    ''
  if (userAgentPlatform.includes('mac') || /mac|iphone|ipad|ipod/i.test(navigator.platform)) {
    return 'Command'
  }
  return 'Ctrl'
})()
...
const serverSubmitDisabled = computed(() => {
  ...
  if (serverForm.authType === 'privateKey') {
    return !serverForm.privateKeyPath.trim()
  }

  return !serverForm.password.trim()
})
...
const updateDialogTitle = computed(() => {
  if (updateDialogMode.value === 'downloaded') {
    return t('settings.update.readyTitle')
  }
  return t('settings.update.availableTitle')
})
const updateDialogConfirmText = computed(() => {
  if (updateDialogMode.value === 'downloaded') {
    return t('settings.update.restartNow')
  }
  if (updateDialogMode.value === 'downloading') {
    return t('settings.update.downloadingAction')
  }
  return t('settings.update.updateNow')
})
const updateDialogConfirmDisabled = computed(() => updateDialogBusy.value || updateDialogMode.value === 'downloading')
const updateProgressPercentText = computed(() => `${Math.round(updateProgressPercent.value)}%`)

function isSettingsTab(value: unknown): value is SettingsTab {
  return value === 'general' || value === 'servers' || value === 'triggers' || value === 'ai'
}

function getElectronSettingsApi(): SettingsApi | null {
  const api = (window as unknown as { electronAPI?: { settings?: SettingsApi } }).electronAPI?.settings
...
async function readPersistedActiveTab(): Promise<SettingsTab | null> {
  const settingsApi = getElectronSettingsApi()
  if (settingsApi) {
    try {
      const result = await settingsApi.get(SETTINGS_ACTIVE_TAB_STORAGE_KEY)
      if (!result.ok) {
        throw new Error(result.error.message)
      }
      const stored = result.data.setting?.value ?? null
      if (isSettingsTab(stored)) {
        try {
          window.localStorage.removeItem(SETTINGS_ACTIVE_TAB_STORAGE_KEY)
        } catch {
  ...
  const raw = window.localStorage.getItem(SETTINGS_ACTIVE_TAB_STORAGE_KEY)
  return isSettingsTab(raw) ? raw : null
}

async function persistActiveTab(tab: SettingsTab): Promise<void> {
  const settingsApi = getElectronSettingsApi()
  if (settingsApi) {
    try {
      const result = await settingsApi.set(SETTINGS_ACTIVE_TAB_STORAGE_KEY, tab)
      if (!result.ok) {
        throw new Error(result.error.message)
      }
      try {
...
watch(
  ...
  (open) => {
    ...
    void Promise.allSettled([triggerState.ensureLoaded(), serverState.refreshServers()]).then((results) => {
      const [triggerResult, serverResult] = results
      if (triggerResult.status === 'fulfilled' && serverResult.status === 'fulfilled') {
    ...
    if (props.initialTab) {
      ...
      void readPersistedActiveTab().then((tab) => {
...
watch(
  ...
  (tab) => {
    ...
    void persistActiveTab(tab)
    if (tab === 'servers') {
      void serverState.refreshServers().then(() => {
        if (props.open) {
          reconcileHiddenServerTriggers()
        }
      }).catch(() => {
        // Keep lastError in serverState; empty/error UI is driven by loading/list state.
      })
    }
    if (tab === 'ai') {
      void loadAIPlatforms()
...
watch(
  ...
  (open, wasOpen) => {
    if (open) {
      void syncNativeUpdateState()
      document.addEventListener('keydown', onEscape, true)
      return
    }
    document.removeEventListener('keydown', onEscape, true)
    if (wasOpen) {
      // Ensure any open AI command bars pick up the latest platform list after settings closes.
      window.dispatchEvent(new CustomEvent(AI_PLATFORMS_CHANGED_EVENT))
    }
  },
  { immediate: true },
)
...
onMounted(() => {
  ...
  if (updateApi && typeof updateApi.onStateChanged === 'function') {
    updateStateUnsubscribe = updateApi.onStateChanged((state) => {
      applyNativeUpdateState(state)
    })
  }
  void syncNativeUpdateState()
})
...
onBeforeUnmount(() => {
  document.removeEventListener('keydown', onEscape, true)
  updateStateUnsubscribe?.()
  updateStateUnsubscribe = null
})
...
function applyNativeUpdateState(state: NativeUpdateState): void {
  updateCurrentVersion.value = state.currentVersion
  updateLatestVersion.value = state.latestVersion ?? ''
  updateLatestTag.value = state.latestTag ?? ''
  updateReleaseUrl.value = state.releaseUrl
  updateProgressPercent.value = state.progressPercent
  const shouldShowDialog = props.open
  if (!shouldShowDialog) {
    updateDialogOpen.value = false
  }
  ...
  if (state.phase === 'available') {
    ...
    updateDialogMode.value = 'confirm'
  ...
  if (state.phase === 'downloading') {
    ...
    updateDialogBusy.value = false
    updateDialogError.value = null
    updateStatusMessage.value = t('settings.update.downloadingStatus')
    return
  }
  ...
  if (state.phase === 'downloaded') {
    ...
    updateDialogMode.value = 'downloaded'
    updateDialogOpen.value = true
    updateDialogBusy.value = false
    updateDialogError.value = null
    updateStatusMessage.value = t('settings.update.downloadedStatus')
    return
  }
  ...
  if (state.phase === 'error') {
    ...
    updateDialogMode.value = state.latestVersion ? 'confirm' : updateDialogMode.value
    updateDialogOpen.value = Boolean(state.latestVersion)
    updateDialogBusy.value = false
    updateDialogError.value = state.errorMessage || t('settings.update.downloadFailed')
    updateStatusMessage.value = updateDialogError.value
  }
}

async function syncNativeUpdateState(): Promise<void> {
  const updateApi = resolveUpdateApi()
  if (!updateApi || typeof updateApi.getState !== 'function') {
    return
  }
  const result = await updateApi.getState()
  if (!result.ok) {
    return
  }
  applyNativeUpdateState(result.data)
}

async function checkForAppUpdates(): Promise<void> {
  const updateApi = resolveUpdateApi()
  if (!updateApi) {
    updateStatusMessage.value = t('settings.update.unavailable')
    return
  }
  ...
  try {
    ...
    if (!checked.data.hasUpdate) {
      updateStatusMessage.value = t('settings.update.latest')
      closeUpdateDialog()
      return
    }

    if (checked.data.managedByNativeUpdater) {
      updateCurrentVersion.value = checked.data.currentVersion
      updateLatestVersion.value = checked.data.latestVersion
      updateLatestTag.value = checked.data.latestTag
      updateReleaseUrl.value = checked.data.releaseUrl
    ...
    if (prompted.data.action === 'update' && prompted.data.openedReleasePage) {
      updateStatusMessage.value = t('settings.update.opened')
    }
  } catch (error) {
    updateStatusMessage.value = getErrorMessage(error, t('settings.update.checkFailed'))
  } finally {
    updateChecking.value = false
  }
}

async function startNativeUpdateDownload(): Promise<void> {
  const updateApi = resolveUpdateApi()
  if (!updateApi || typeof updateApi.download !== 'function') {
    updateDialogError.value = t('settings.update.unavailable')
    return
  }
  ...
  try {
    ...
    updateDialogError.value = getErrorMessage(error, t('settings.update.downloadFailed'))
    updateStatusMessage.value = updateDialogError.value
    updateDialogMode.value = 'confirm'
  } finally {
    updateDialogBusy.value = false
...
async function restartToInstallUpdate(): Promise<void> {
  const updateApi = resolveUpdateApi()
  if (!updateApi || typeof updateApi.restartToInstall !== 'function') {
    updateDialogError.value = t('settings.update.unavailable')
    return
  }
  updateDialogBusy.value = true
  try {
    const result = await updateApi.restartToInstall()
    if (!result.ok) {
      throw new Error(result.error.message)
    }
  } catch (error) {
    updateDialogBusy.value = false
    updateDialogError.value = getErrorMessage(error, t('settings.update.restartFailed'))
    updateStatusMessage.value = updateDialogError.value
  }
}
...
async function submitUpdateDialog(): Promise<void> {
  ...
  await startNativeUpdateDownload()
}
...
function openEditServerEditor(server: ServerRecord): void {
  ...
  serverForm.privateKeyPath = server.privateKeyPath ?? ''
  serverForm.passphrase = server.passphrase ?? ''
}
...
async function submitServerEditor(): Promise<void> {
  ...
  try {
    ...
    const authType = serverForm.authType
    const payload: CreateServerInput = {
      ...
      authType,
      password: authType === 'password' ? serverForm.password : null,
      privateKeyPath: authType === 'privateKey' ? serverForm.privateKeyPath.trim() : null,
      passphrase: authType === 'privateKey' ? serverForm.passphrase.trim() || null : null,
    }
    ...
    if (authType === 'password' && !payload.password?.trim()) {
      throw new Error(t('settings.server.error.passwordRequired'))
    }
    if (authType === 'privateKey' && !payload.privateKeyPath?.trim()) {
      throw new Error(t('settings.server.error.privateKeyRequired'))
    }
...
function buildServerPasswordPromptPatterns(server: Pick<ServerRecord, 'username' | 'host'>): string[] {
...
function upsertHiddenServerTrigger(server: ServerRecord): void {
  ...
  const patterns = buildServerPasswordPromptPatterns({ username, host })
  const existingRuleIds = findHiddenServerTriggerRuleIds(server.id)
  const reusableRuleIds = [...existingRuleIds]

  for (const pattern of patterns) {
    const ruleId = reusableRuleIds.shift()
    if (!ruleId) {
      triggerState.addRule({
        pattern,
        sendText: password,
  ...
  for (const staleRuleId of reusableRuleIds) {
    triggerState.removeRule(staleRuleId)
  }
}

function reconcileHiddenServerTriggers(): void {
  triggerState.syncHiddenServerRules(serverState.servers.value)
}
</script>
    ...
    <div v-if="props.open" class="global-settings-mask" @click.self="emit('close')">
      <section class="global-settings-dialog" role="dialog" aria-modal="true">
          ...
          <section v-if="activeTab === 'general'" class="tab-pane general-tab">
            ...
            <div class="general-field general-field--action">
              ...
              <div class="general-update">
                ...
                <p v-if="updateStatusMessage" class="general-update__status">{{ updateStatusMessage }}</p>
              </div>
            </div>
        ...
        <AppDialog
          ...
          :confirm-text="updateDialogConfirmText"
          :cancel-text="t('settings.cancel')"
          :confirm-disabled="updateDialogConfirmDisabled"
          @close="closeUpdateDialog"
          @confirm="void submitUpdateDialog()"
        >
          <div class="update-dialog">
            <p class="update-dialog__headline">
              {{
                updateDialogMode === 'downloaded'
                  ? t('settings.update.readyDescription')
                  : t('settings.update.availableDescription')
              }}
            </p>
            <dl class="update-dialog__meta">
              <div>
                <dt>{{ t('settings.update.currentVersion') }}</dt>
                <dd>{{ updateCurrentVersion || '—' }}</dd>
              </div>
              <div>
                <dt>{{ t('settings.update.latestVersion') }}</dt>
                <dd>{{ updateLatestVersion || '—' }}</dd>
              </div>
              <div v-if="updateLatestTag">
                <dt>{{ t('settings.update.releaseTag') }}</dt>
                <dd>{{ updateLatestTag }}</dd>
              </div>
            </dl>

            <div v-if="updateDialogMode === 'downloading' || updateDialogMode === 'downloaded'" class="update-progress">
              <div class="update-progress__track" aria-hidden="true">
                <span class="update-progress__bar" :style="{ width: updateProgressPercentText }"></span>
              </div>
              <div class="update-progress__summary">
                <strong>{{ updateProgressPercentText }}</strong>
                <span>
                  {{
                    updateDialogMode === 'downloaded'
                      ? t('settings.update.downloadedStatus')
                      : t('settings.update.downloadingStatus')
                  }}
                </span>
              </div>
            </div>

            <p v-if="updateDialogError" class="form-error">{{ updateDialogError }}</p>
          </div>
        </AppDialog>
        ...
        >
          <form class="server-editor-form" @submit.prevent="void submitServerEditor()">
            ...
            <label>
              <span>{{ t('settings.server.field.authType') }}</span>
              <select v-model="serverForm.authType">
                <option value="password">{{ t('settings.server.authType.password') }}</option>
                <option value="privateKey">{{ t('settings.server.authType.privateKey') }}</option>
              </select>
            </label>
            <label v-if="serverForm.authType === 'password'">
              <span>
            ...
            <template v-else>
              <label>
                <span>
                  {{ t('settings.server.field.privateKeyPath') }}
                  <em class="server-editor-form__required" aria-hidden="true">*</em>
                </span>
                <input v-model="serverForm.privateKeyPath" type="text" />
              </label>
              <label>
                <span>{{ t('settings.server.field.passphrase') }}</span>
                <input v-model="serverForm.passphrase" type="password" />
              </label>
            </template>
...
.update-dialog__meta dt {
  color: #64748b;
  font-size: 12px;
}

.update-dialog__meta dd {
  margin: 0;
  color: #0f172a;
  font-size: 13px;
