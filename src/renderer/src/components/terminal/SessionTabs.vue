...
const isMacOS =
  typeof navigator !== 'undefined' &&
  /mac|iphone|ipad|ipod/i.test(navigatorPlatform)
...
type UpdateApi = {
  getState?: () => Promise<{ ok: boolean; data: NativeUpdateState }>
  restartToInstall?: () => Promise<{ ok: boolean }>
  onStateChanged?: (listener: (state: NativeUpdateState) => void) => Unsubscribe
}

function resolveUpdateApi(): UpdateApi | null {
  const target = window as Window & {
    electronAPI?: { update?: UpdateApi }
    __electronAPIBridge?: { update?: UpdateApi }
  }
  return target.electronAPI?.update ?? target.__electronAPIBridge?.update ?? null
}

function applyUpdateState(state: NativeUpdateState): void {
  updateReady.value = state.phase === 'downloaded'
}

async function syncUpdateState(): Promise<void> {
  const updateApi = resolveUpdateApi()
  if (!updateApi?.getState) {
    return
  }
  const result = await updateApi.getState()
  if (result.ok) {
    applyUpdateState(result.data)
  }
}

async function restartToInstall(): Promise<void> {
  const updateApi = resolveUpdateApi()
  if (!updateApi?.restartToInstall) {
    return
  }
  if (updateRestarting.value) {
    return
  }
...
<template>
  <div class="session-tabs" :class="{ 'is-macos': isMacOS }">
    <div class="tab-list">
      <div
    ...
    <div ref="downloadContainerRef" class="download-center">
      ...
      v-if="updateReady"
        ...
        :aria-label="t('terminal.update.restartAria')"
        :title="t('terminal.update.restartTitle')"
        @click.stop="restartToInstall"
      >
        {{ t('terminal.update.restartLabel') }}
      </button>
...
.session-tabs.is-macos .tab-list {
  padding-left: 80px;
}
...
.update-ready-btn:hover:not(:disabled) {
  background: #fecaca;
  border-color: #f87171;
}

.update-ready-btn:disabled {
  cursor: not-allowed;
  opacity: 0.6;
}
