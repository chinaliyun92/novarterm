...
const WORKSPACE_RESTORE_CLEANUP_DELAY_MS = 10_000
const WORKSPACE_RESTORE_CLEANUP_DEBUG_ENABLED = true

const workspace = useTerminalWorkspaceState()

function logWorkspaceRestoreCleanupDebug(event: string, detail?: Record<string, unknown>): void {
  if (!WORKSPACE_RESTORE_CLEANUP_DEBUG_ENABLED) {
    return
  }
  const timestamp = new Date().toISOString()
  if (detail) {
    console.log(`[workspace-cleanup][${timestamp}] ${event}`, detail)
    return
  }
  console.log(`[workspace-cleanup][${timestamp}] ${event}`)
}
...
onMounted(() => {
  const electronApi = (window as Window & {
    electronAPI?: {
      settings?: { cleanCommandBarHistory: (req: { keepSessionIds: string[] }) => Promise<{ ok: boolean }> }
      terminal?: { cleanPersistedLogs: (req: { keepSessionIds: string[] }) => Promise<{ ok: boolean }> }
    }
  }).electronAPI
  const settingsApi = electronApi?.settings
  const terminalApi = electronApi?.terminal
  if (!settingsApi?.cleanCommandBarHistory && !terminalApi?.cleanPersistedLogs) {
    return
  }
  cleanupTimer = setTimeout(() => {
    cleanupTimer = null
    const keepSessionIds = workspace.sessions.value.map((s) => s.id)
    logWorkspaceRestoreCleanupDebug('cleanup-triggered', {
      keepSessionIds,
      sessionCount: keepSessionIds.length,
    })
    if (settingsApi?.cleanCommandBarHistory) {
      void settingsApi.cleanCommandBarHistory({ keepSessionIds }).catch(() => {})
    }
    if (terminalApi?.cleanPersistedLogs) {
      void terminalApi.cleanPersistedLogs({ keepSessionIds })
        .then((result) => {
          logWorkspaceRestoreCleanupDebug('terminal-log-cleanup-finished', {
            keepSessionIdsCount: keepSessionIds.length,
            ok: result.ok,
            removedCount: result.ok ? result.data.removedCount : null,
            error: result.ok ? null : result.error,
          })
        })
        .catch((error) => {
          logWorkspaceRestoreCleanupDebug('terminal-log-cleanup-exception', {
            error: error instanceof Error ? error.message : String(error),
          })
        })
    }
  }, WORKSPACE_RESTORE_CLEANUP_DELAY_MS)
})
onBeforeUnmount(() => {
