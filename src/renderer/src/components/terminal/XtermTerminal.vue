...
import type { TerminalPrepareForQuitEvent } from '../../../../shared/types/terminal'
...
import { useTerminalTriggerRuntime } from '../../composables/useTerminalTriggerRuntime'
...
const props = defineProps<{
  sessionId: string
  dropIndicatorPosition?: 'top' | 'left' | 'right' | 'bottom' | null
}>()
...
let removeBridgePrepareForQuitListener: (() => void) | null = null
...
let removeAICommandBarShortcutListener: (() => void) | null = null
let removeAIPlatformsChangedListener: (() => void) | null = null
...
let inlineAIImeSuppressUntil = 0
const fontZoomHint = ref('')
let fontZoomHintTimer: ReturnType<typeof setTimeout> | null = null
let prepareForQuitListenerRefCount = 0
let prepareForQuitFlushPromise: Promise<void> | null = null
type AIChatMessage =
...
const AI_PROMPT_RECENT_CONVERSATION_MESSAGES = 5
...
const AI_PLATFORMS_CHANGED_EVENT = 'novarterm:ai-platforms-changed'
const AI_COMMAND_BAR_DEBUG_LOG_ENABLED = import.meta.env.DEV
const TERMINAL_REPLAY_DEBUG_LOG_ENABLED = true
const isWindowsLikePlatform = (() => {
...
const TERMINAL_DEBUG_HINT_SUPPRESSED_PATTERNS: RegExp[] = [
  /\bcommand not found\b/i,
...
const aiModelTargets = ref<AIModelTarget[]>([])
const aiSelectedModelTargetId = ref<string>('')
const aiHasConfiguredModel = computed(() => aiModelTargets.value.length > 0)
...
const TRANSCRIPT_REPLAY_INPUT_SUPPRESS_MS = 180
const INLINE_AI_IME_SUPPRESS_MS = 260
const ALT_SCREEN_TOGGLE_SEQUENCE_REGEX = /\u001b\[\?(1049|1047|47)([hl])/g
...
const terminalAltScreenActiveBySession = new Set<string>()
...
const TRIGGER_OUTPUT_SETTLE_MS = 300
const TRIGGER_AUTO_INPUT_DELAY_MS = 100
const NEW_PANE_LOCALHOST_ITEM_ID = 'localhost'
let transcriptReplayInputSuppressedUntil = 0
...
const REPLAY_TRANSCRIPT_DEBUG_LOG_MAX_CHARS = 12000

function stripTerminalControlSequences(value: string): string {
  return value
    // Strip OSC (ESC ] ... BEL / ST), CSI (ESC [ ...), DCS/PM/APC (ESC P/^/_ ... ST), and 2-byte escapes.
    .replace(/\u001b\][^\u0007\u001b]*(?:\u0007|\u001b\\)/g, '')
    .replace(/\u001b\[[0-?]*[ -/]*[@-~]/g, '')
    .replace(/\u001b[PX^_][\s\S]*?\u001b\\/g, '')
    .replace(/\u001b[@-Z\\-_]/g, '')
...
function stripInvisibleDebugCharacters(value: string): string {
  return value
    // Remove C0/C1 controls except visible escapes already converted by formatShellOutputChunk.
    .replace(/[\u0000-\u0008\u000B\u000C\u000E-\u001F\u007F-\u009F]/g, '')
    // Remove common zero-width and BOM characters.
    .replace(/[\u200B-\u200D\u2060\uFEFF]/g, '')
}

function buildReplayTranscriptDebugData(transcript: string): Record<string, unknown> {
  const cleanedTranscript = stripTerminalControlSequences(transcript)
  const normalized = stripInvisibleDebugCharacters(formatShellOutputChunk(cleanedTranscript))
  if (normalized.length <= REPLAY_TRANSCRIPT_DEBUG_LOG_MAX_CHARS) {
    return {
      transcriptData: normalized,
      transcriptDataTruncated: false,
    }
  }

  const headChars = Math.floor(REPLAY_TRANSCRIPT_DEBUG_LOG_MAX_CHARS / 2)
  const tailChars = REPLAY_TRANSCRIPT_DEBUG_LOG_MAX_CHARS - headChars
  const omittedChars = normalized.length - REPLAY_TRANSCRIPT_DEBUG_LOG_MAX_CHARS
  return {
    transcriptData: `${normalized.slice(0, headChars)} ...[truncated ${omittedChars} chars]... ${normalized.slice(-tailChars)}`,
    transcriptDataTruncated: true,
    transcriptDataLoggedChars: REPLAY_TRANSCRIPT_DEBUG_LOG_MAX_CHARS,
    transcriptDataTotalChars: normalized.length,
  }
}
...
function logTerminalReplayDebug(event: string, detail?: Record<string, unknown>): void {
  if (!TERMINAL_REPLAY_DEBUG_LOG_ENABLED) {
    return
  }

  const timestamp = new Date().toISOString()
  if (detail) {
    console.log(`[terminal-replay][${timestamp}] ${event}`, detail)
    return
  }
  console.log(`[terminal-replay][${timestamp}] ${event}`)
}
...
const TERMINAL_MONOSPACE_FALLBACK =
  '"Sarasa Mono SC","Noto Sans Mono CJK SC","JetBrains Mono","SF Mono",Menlo,Monaco,Consolas,"Liberation Mono","Courier New",monospace'
...
function normalizePersistedTerminalStatePayload(payload: unknown): { transcript: string; cwd: string | null } {
  if (!payload || typeof payload !== 'object') {
    return { transcript: '', cwd: null }
  }
...
function summarizePersistedTerminalStatePayload(payload: { transcript: string; cwd: string | null }): Record<string, unknown> {
  return {
...
function resolveAIFallbackLanguageLabelFromUiLanguage(): string {
  return i18n.locale.value === 'zh-CN' ? 'Simplified Chinese' : 'English'
}

function getCurrentAIModelTarget(): AIModelTarget | null {
  return (
...
async function loadAIModelTargets(): Promise<void> {
  ...
  aiModelTargets.value = userTargets

  const persistedSelected = (await getAISettingValue(AI_SELECTED_MODEL_KEY))?.trim()
  const fallbackId = aiModelTargets.value[0]?.id ?? ''
  const selectedId =
  ...
  if (!selectedId) {
    aiCommandBarLastErrorCode.value = 'not_configured'
  } else if (aiCommandBarLastErrorCode.value === 'not_configured') {
    aiCommandBarLastErrorCode.value = null
  }
}

async function ensureAIConfigured(options?: { openSettings?: boolean }): Promise<boolean> {
  await loadAIModelTargets()
  if (aiModelTargets.value.length > 0) {
    return true
  }
  aiCommandBarLastErrorCode.value = 'not_configured'
  globalMessage.info(t('terminal.aiBar.error.notConfigured'), { replace: true })
...
async function refreshAIModelTargetsFromSettings(): Promise<void> {
  await loadAIModelTargets()
  if (!aiCommandBarVisible.value) {
    return
  }
  if (aiHasConfiguredModel.value) {
    nextTick(() => {
...
function buildAIGlobalSystemPrompt(fallbackLanguageLabel: string): string {
  const shellHint = runtimeSettings.defaultShell.value.trim()
  const platformLabel = isWindowsLikePlatform ? 'Windows' : 'macOS/Linux'
  const shellLabel = shellHint ? `默认 shell：${shellHint}` : '默认 shell：未知'
  return `你是一个面向终端用户的助手。
当前环境：${platformLabel}，${shellLabel}。
根据会话记录中“用户消息”的语言自动判断回复语言，并保持一致。
如果会话记录里出现多种语言，优先使用最近一条用户消息的语言。
如果没有可用于判断语言的用户消息，再使用${fallbackLanguageLabel}回复。
命令和代码必须保持不变。
解释要简洁。`
}
...
function buildAIExplainCommandSystemPrompt(command: string, terminalContext: string): string {
  return `解释这条shell命令：
${command}
...
function buildAIExplainSelectionSystemPrompt(selection: string, terminalContext: string): string {
  return `解释这段已选中的终端文本：
${selection}
...
2) 如果文本看起来像日志/错误/输出：结合上下文总结其含义、可能原因，并给出下一步排查建议。`
}

function buildAIExplainErrorSystemPrompt(context: string, command: string, terminalContext: string): string {
  return `解释这次命令失败信息：
${context}
...
function buildAIDebugSystemPrompt(context: string, latestCommand: string, terminalContext: string): string {
  return `调试这段终端输出：
${context}
...
/**
 * AI Prompt 规则（当前约定，后续改动需先同步产品规则）：
 * 1) ACB 中直接提问：附带最近 5 条 ACB 会话记录。
 * 2) ACB 里点 explain/expand：附带最近 5 条 ACB 会话记录，附带终端上下文。
 * 3) 终端选择文本 explain：附带最近 5 条 ACB 会话记录，附带终端上下文。
 * 4) 终端异常结束 explain：附带最近 5 条 ACB 会话记录，附带终端上下文。
 * 5) 终端异常结束 debug with novar：附带最近 5 条 ACB 会话记录，附带终端上下文。
 * 6) 终端内辅助生成命令：不附带会话记录，不附带终端上下文。
 */
async function submitAICommandBar(): Promise<void> {
  ...
  const recentContext = getRecentAIConversationMessagesForPrompt()
...
function getRecentAIConversationMessagesForPrompt(): AIChatMessage[] {
  return aiCommandBarMessages.value.slice(-AI_PROMPT_RECENT_CONVERSATION_MESSAGES)
}

function resolveTerminalContextForAISession(sessionId: string | null | undefined): string {
  const normalizedSessionId = sessionId?.trim() ?? ''
  if (!normalizedSessionId) {
    return '(none)'
  }
  return buildTerminalTranscriptContextForAI(normalizedSessionId)
}
...
function onAICmdExpand(command: string): void {
  ...
  void (async () => {
    ...
    const recentContext = getRecentAIConversationMessagesForPrompt()
    const terminalContext = resolveTerminalContextForAISession(getAICurrentSessionId())
    ...
    await sendAIMessagesAndStream([...recentContext, displayMessage], {
      scenarioSystemPrompt: buildAIExplainCommandSystemPrompt(command, terminalContext),
    })
  })()
}
...
async function askAIAndStreamResponseWithMessages(
  ...
  const fallbackLanguageLabel = resolveAIFallbackLanguageLabelFromUiLanguage()
  const systemPrompt = buildAIGlobalSystemPrompt(fallbackLanguageLabel)
  ...
  if (!selectedTarget) {
    throw new AIRequestError(t('terminal.aiBar.error.notConfigured'), 'not_configured')
  }
...
function suppressReplayTriggeredInputTemporarily(): void {
  const nextSuppressedUntil = Date.now() + TRANSCRIPT_REPLAY_INPUT_SUPPRESS_MS
  if (nextSuppressedUntil > transcriptReplayInputSuppressedUntil) {
    transcriptReplayInputSuppressedUntil = nextSuppressedUntil
  }
  logTerminalReplayDebug('replay-suppress-window-updated', {
    suppressUntil: new Date(transcriptReplayInputSuppressedUntil).toISOString(),
    suppressDurationMs: TRANSCRIPT_REPLAY_INPUT_SUPPRESS_MS,
  })
}

function isReplaySuppressionWindowActive(): boolean {
  return Date.now() <= transcriptReplayInputSuppressedUntil
}

function isReplayTriggeredTerminalQueryInput(data: string): boolean {
  if (!data || Date.now() > transcriptReplayInputSuppressedUntil) {
    return false
  }

  return stripTerminalQueryResponseSequences(data) !== data
}

function stripTerminalQueryResponseSequences(data: string): string {
  return data
    // OSC color/style query responses, e.g. ESC ] 10;rgb:... BEL
    .replace(/\x1b\](?:10|11|12|4;[0-9]+);[^\u0007\x1b]*(?:\u0007|\x1b\\)/g, '')
    .replace(/\u009d(?:10|11|12|4;[0-9]+);[^\u0007\x1b]*(?:\u0007|\x1b\\)/g, '')
    // Cursor position report, e.g. ESC [ 12 ; 34 R
    .replace(/\x1b\[[0-9]+;[0-9]+R/g, '')
    .replace(/\u009b[0-9]+;[0-9]+R/g, '')
    // Device attributes reports, e.g. ESC [ > 0 ; 276 ; 0 c or ESC [ ? 1 ; 2 c
    .replace(/\x1b\[(?:>|\\?)[0-9;]*c/g, '')
    .replace(/\u009b(?:>|\\?)[0-9;]*c/g, '')
    // Mode/status query reports, e.g. ESC [ 12 ; 2 $ y or ESC [ 12 ; 2 y
    .replace(/\x1b\[[0-9;?]*(?:\$)?y/g, '')
    .replace(/\u009b[0-9;?]*(?:\$)?y/g, '')
}
...
function markTerminalCommandTraceInterruptedByUser(sessionId: string): void {
  const trace = terminalCommandTraceBySession.get(sessionId)
  if (!trace) {
    return
  }
  trace.interruptedByUser = true
}

function isUserInterruptedTerminalCommand(trace: TerminalCommandTrace): boolean {
  return trace.interruptedByUser
}
...
function finalizeTerminalCommandTrace(sessionId: string): void {
  ...
  // Terminal auto error detection (Explain Error / Debug With AI) is temporarily disabled.
}
...
function shouldSuppressTerminalDebugHint(trace: TerminalCommandTrace): boolean {
  const outputLine = pickTerminalDebugOutputLine(trace.output)
  if (!outputLine) {
    return false
  }

  return TERMINAL_DEBUG_HINT_SUPPRESSED_PATTERNS.some((pattern) => pattern.test(outputLine))
}
...
function buildTerminalTranscriptContextForAI(sessionId: string): string {
  const transcript = sessionCache.getTranscript(sessionId)
  if (!transcript) {
    return '(none)'
  }

  const normalized = redactSensitiveTerminalDebugText(toTerminalDebugPlainText(transcript))
  ...
  return joined.length > TERMINAL_DEBUG_CONTEXT_MAX_CHARS
    ? joined.slice(joined.length - TERMINAL_DEBUG_CONTEXT_MAX_CHARS)
    : joined
}
...
function maybeShowTerminalDebugHint(sessionId: string, trace: TerminalCommandTrace): void {
  ...
  if (shouldSuppressTerminalDebugHint(trace)) {
    return
  }
  if (aiCommandBarVisible.value) {
    return
...
async function explainErrorWithPayload(payload: TerminalDebugHintPayload): Promise<void> {
  ...
  const recentContext = getRecentAIConversationMessagesForPrompt()
  const terminalContext = resolveTerminalContextForAISession(sessionId)
  const displayMessage = buildDebugHintDisplayMessage('terminal.debugHint.explainUserMessage', outputLine, command)
  aiCommandBarMessages.value = [...aiCommandBarMessages.value, displayMessage]
  dismissTerminalDebugHint()
  await sendAIMessagesAndStream([...recentContext, displayMessage], {
    scenarioSystemPrompt: buildAIExplainErrorSystemPrompt(context, command, terminalContext),
  })
}
...
async function debugErrorWithPayload(payload: TerminalDebugHintPayload): Promise<void> {
  ...
  dismissTerminalDebugHint()
  await sendAIMessagesAndStream([...recentContext, displayMessage], {
    scenarioSystemPrompt: buildAIDebugSystemPrompt(context, command, terminalContext),
  })
}
...
async function clearInlineAIInput(sessionId: string): Promise<boolean> {
  const payload = shouldUseWindowsInlineAIClear(sessionId) ? '\u0003' : '\u0015'
  return writeInputToSession(sessionId, payload, {
    trackHistory: false,
...
async function runInlineAICommandRequest(sessionId: string, promptText: string): Promise<void> {
  ...
  if (!(await ensureAIConfigured({ openSettings: true }))) {
    await refreshTerminalPromptLine(sessionId)
    return
  }
  ...
  try {
    const clearOk = await clearInlineAIInput(sessionId)
    if (!clearOk) {
      return
    }
...
function clearTriggerRuntimeState(sessionId?: string): void {
  if (sessionId) {
    const normalizedSessionId = sessionId.trim()
    if (!normalizedSessionId) {
      return
    }
    triggerArmedBySession.delete(normalizedSessionId)
    triggerLastLineBySession.delete(normalizedSessionId)
    const outputTimer = triggerOutputTimerBySession.get(normalizedSessionId)
    if (outputTimer) {
      clearTimeout(outputTimer)
      triggerOutputTimerBySession.delete(normalizedSessionId)
    }
    const autoInputTimer = triggerAutoInputTimerBySession.get(normalizedSessionId)
    if (autoInputTimer) {
      clearTimeout(autoInputTimer)
      triggerAutoInputTimerBySession.delete(normalizedSessionId)
    }
    return
  }

  triggerArmedBySession.clear()
  triggerLastLineBySession.clear()
  for (const timer of triggerOutputTimerBySession.values()) {
    clearTimeout(timer)
  }
  triggerOutputTimerBySession.clear()
  for (const timer of triggerAutoInputTimerBySession.values()) {
    clearTimeout(timer)
  }
  triggerAutoInputTimerBySession.clear()
}
...
function buildHiddenServerTriggerPatterns(server: Pick<ServerRecord, 'username' | 'host'>): string[] {
  ...
  return variants
    .map((pattern) => normalizeTriggerText(pattern))
    .filter((pattern, index, collection) => pattern.length > 0 && collection.indexOf(pattern) === index)
}

function matchesTriggerOutput(
  rule: (typeof triggerState.rules.value)[number],
  normalizedOutput: string,
): boolean {
  const normalizedPattern = normalizeTriggerText(rule.pattern?.trim() ?? '')
  if (normalizedPattern && normalizedOutput.endsWith(normalizedPattern)) {
    return true
  }

  if (rule.source !== 'server_hidden' || typeof rule.sourceServerId !== 'number') {
    return false
  }

  const sourceServer = serverState.servers.value.find((server) => server.id === rule.sourceServerId)
  if (!sourceServer) {
    return false
  }

  return buildHiddenServerTriggerPatterns(sourceServer).some((pattern) => normalizedOutput.endsWith(pattern))
}
...
function resolveTriggerSendPayload(sendText: string, autoSend: boolean): string {
...
function armTriggerForSession(sessionId: string): void {
  const normalizedSessionId = sessionId.trim()
  if (!normalizedSessionId) {
    return
  }
  triggerArmedBySession.add(normalizedSessionId)
  triggerLastLineBySession.delete(normalizedSessionId)
  const outputTimer = triggerOutputTimerBySession.get(normalizedSessionId)
  if (outputTimer) {
    clearTimeout(outputTimer)
...
function scheduleTriggerOutputMatch(sessionId: string): void {
  const timer = triggerOutputTimerBySession.get(sessionId)
  if (timer) {
    clearTimeout(timer)
  }

  const nextTimer = setTimeout(() => {
    triggerOutputTimerBySession.delete(sessionId)
    void evaluateTriggerOutputMatch(sessionId)
  }, TRIGGER_OUTPUT_SETTLE_MS)
  triggerOutputTimerBySession.set(sessionId, nextTimer)
}

function appendTriggerOutput(sessionId: string, chunk: string): void {
  const normalizedSessionId = sessionId.trim()
  if (!normalizedSessionId || !chunk) {
    return
  }
  if (!triggerArmedBySession.has(normalizedSessionId)) {
    return
  }
  const cleaned = stripAnsiEscapeSequences(chunk).replace(/\r/g, '\n')
  if (!cleaned) {
    return
  }
  const previous = triggerLastLineBySession.get(normalizedSessionId) ?? ''
  ...
  if (cleaned.includes('\n')) {
    ...
    nextLine = `${previous}${cleaned}`
  }
  if (nextLine) {
    triggerLastLineBySession.set(normalizedSessionId, nextLine)
  }
  scheduleTriggerOutputMatch(normalizedSessionId)
}

async function sendTriggerAutoInput(sessionId: string, payload: string): Promise<void> {
  const wrote = await writeInputToSession(sessionId, payload, {
    trackHistory: false,
    reportUnavailable: false,
  })
  if (wrote && (payload.includes('\r') || payload.includes('\n'))) {
    armTriggerForSession(sessionId)
  }
}

function scheduleTriggerAutoInput(sessionId: string, payload: string): void {
  const existing = triggerAutoInputTimerBySession.get(sessionId)
  if (existing) {
    clearTimeout(existing)
    triggerAutoInputTimerBySession.delete(sessionId)
  }

  const timer = setTimeout(() => {
    triggerAutoInputTimerBySession.delete(sessionId)
    void sendTriggerAutoInput(sessionId, payload)
  }, TRIGGER_AUTO_INPUT_DELAY_MS)
  triggerAutoInputTimerBySession.set(sessionId, timer)
}

async function evaluateTriggerOutputMatch(sessionId: string): Promise<void> {
  if (!triggerArmedBySession.has(sessionId)) {
    return
  }

  const lastLine = triggerLastLineBySession.get(sessionId) ?? ''
  if (!lastLine.trim()) {
    return
  }

  const normalizedOutput = normalizeTriggerText(lastLine)
  if (!normalizedOutput) {
    return
  }

  let matchedRule: (typeof triggerState.rules.value)[number] | null = null
  for (const rule of triggerState.rules.value) {
    if (!rule.enabled) {
      continue
    }
    const pattern = rule.pattern?.trim() ?? ''
    if (!pattern || !rule.sendText) {
      continue
    }
    if (!normalizeTriggerText(pattern)) {
      continue
    }
    if (matchesTriggerOutput(rule, normalizedOutput)) {
      matchedRule = rule
      break
    }
  }
  ...
  const payload = resolveTriggerSendPayload(matchedRule.sendText, matchedRule.autoSend)
  if (!payload) {
    return
  }

  triggerArmedBySession.delete(sessionId)
  triggerLastLineBySession.delete(sessionId)
  const outputTimer = triggerOutputTimerBySession.get(sessionId)
  if (outputTimer) {
    clearTimeout(outputTimer)
    triggerOutputTimerBySession.delete(sessionId)
  }
  scheduleTriggerAutoInput(sessionId, payload)
}
...
const SSH_NON_INTERACTIVE_OPTIONS = new Set<string>([
  '-f',
  '-N',
  '-n',
  '-T',
...
function shouldTreatSshCommandAsInteractiveShell(command: string): boolean {
  const tokens = tokenizeShellCommand(command.trim())
  if (!tokens.length) {
    return false
  }
  ...
  for (; index < tokens.length; index += 1) {
    ...
    if (token === '--') {
      destinationIndex = index + 1 < tokens.length ? index + 1 : -1
      break
    }

    if (token.startsWith('-') && token.length > 1) {
      if (SSH_NON_INTERACTIVE_OPTIONS.has(token)) {
        return false
      }
  ...
  if (destinationIndex < 0 || destinationIndex >= tokens.length) {
    return false
  }

  // `ssh host command` is a one-shot remote command, not an interactive shell.
  if (destinationIndex + 1 < tokens.length) {
    return false
  }
...
function preparePastedText(text: string): string | null {
  ...
  // Keep paste behavior predictable: always preserve raw clipboard text.
  return text
}
...
async function persistLocalSessionStateNow(
  ...
  if (!persistedCwd) {
    const transcriptHint = resolvePromptCwdHintFromTranscript(sessionId)
    if (isLikelyPersistableLocalCwd(transcriptHint)) {
      persistedCwd = transcriptHint
    }
  }
  ...
  await invokeBridge('saveSnapshot', () =>
    saveSnapshot({
      sessionId,
      snapshot,
...
async function flushOpenedSessionsForQuit(): Promise<void> {
  clearDelayedCwdRefreshTimer()

  const sessionIds = Array.from(openedBridgeSessionIds)
...
async function handlePrepareForQuit(event: TerminalPrepareForQuitEvent): Promise<void> {
  ...
  const bridge = getTerminalBridge()
  const prepareForQuitComplete = bridge?.prepareForQuitComplete
  if (!prepareForQuitComplete) {
    return
  }

  if (!prepareForQuitFlushPromise) {
    prepareForQuitFlushPromise = flushOpenedSessionsForQuit().finally(() => {
      prepareForQuitFlushPromise = null
    })
  }

  try {
    await prepareForQuitFlushPromise
  } finally {
    await invokeBridge('prepareForQuitComplete', () =>
...
function attachPrepareForQuitListener(): void {
  prepareForQuitListenerRefCount += 1
  if (removeBridgePrepareForQuitListener) {
    return
  }

  const bridge = getTerminalBridge()
  if (!bridge?.onPrepareForQuit) {
    return
  }

  removeBridgePrepareForQuitListener = normalizeCleanup(
    bridge.onPrepareForQuit((event) => {
      void handlePrepareForQuit(event)
    }),
  )
}

function detachPrepareForQuitListener(): void {
  if (prepareForQuitListenerRefCount > 0) {
    prepareForQuitListenerRefCount -= 1
  }
  ...
  removeBridgePrepareForQuitListener?.()
  removeBridgePrepareForQuitListener = null
}
...
async function hydrateLocalSessionFromPersistence(sessionId: string): Promise<void> {
  ...
  if (sessionCache.hasHydratedPersistedState(sessionId)) {
    logTerminalReplayDebug('hydrate:skip-already-hydrated', {
      sessionId,
      transcriptChars: sessionCache.getTranscript(sessionId).length,
    })
    return
  }
  ...
  if (!bridge?.loadPersisted) {
    ...
    logTerminalReplayDebug('hydrate:skip-loadPersisted-unavailable', {
      sessionId,
    })
    return
  }

  const beforeTranscriptChars = sessionCache.getTranscript(sessionId).length
  logTerminalReplayDebug('hydrate:start', {
    sessionId,
    beforeTranscriptChars,
  ...
  try {
    ...
    if (isBridgeFailure(result)) {
      ...
      logTerminalReplayDebug('hydrate:bridge-failure', {
        sessionId,
    ...
    logTerminalReplayDebug('hydrate:payload-received', {
      sessionId,
      ...summarizePersistedTerminalStatePayload(payload),
    })
    ...
    sessionCache.markHydratedPersistedState(sessionId)
    logTerminalReplayDebug('hydrate:applied', {
      sessionId,
      beforeTranscriptChars,
      afterTranscriptChars: sessionCache.getTranscript(sessionId).length,
      hintedCwd: sessionPathSync.cwdBySession[sessionId] ?? null,
    })
  } catch (error) {
    writeTerminalError(`loadPersisted failed: ${formatError(error)}`)
    logTerminalReplayDebug('hydrate:exception', {
      sessionId,
...
function resetSessionInteractiveInputState(sessionId: string): void {
  localPromptReadyBySession.delete(sessionId)
  sessionPendingInput.delete(sessionId)
  sessionPromptBuffer.delete(sessionId)
  resetLocalEchoLine(sessionId)
}

function isTerminalAlternateScreenActive(sessionId: string): boolean {
  return terminalAltScreenActiveBySession.has(sessionId)
}

function updateTerminalAlternateScreenState(sessionId: string, data: string): void {
  if (!sessionId || !data) {
    return
  }

  let nextActive = terminalAltScreenActiveBySession.has(sessionId)
  let sawToggle = false
  ALT_SCREEN_TOGGLE_SEQUENCE_REGEX.lastIndex = 0

  for (const match of data.matchAll(ALT_SCREEN_TOGGLE_SEQUENCE_REGEX)) {
    sawToggle = true
    nextActive = match[2] === 'h'
  }
  ...
  const previousActive = terminalAltScreenActiveBySession.has(sessionId)
  if (nextActive) {
    terminalAltScreenActiveBySession.add(sessionId)
  } else {
    terminalAltScreenActiveBySession.delete(sessionId)
  }

  if (previousActive !== nextActive) {
    resetSessionInteractiveInputState(sessionId)
  }
}

function shouldUseWindowsInlineAIClear(sessionId: string): boolean {
  return isWindowsLikePlatform && isLocalSession(sessionId)
}
...
function commitSessionCommand(sessionId: string, rawCommand: string): void {
  ...
  if (looksLikeSshCommand(command) && shouldTreatSshCommandAsInteractiveShell(command)) {
...
function collectSessionCommandHistory(sessionId: string, input: string): void {
  if (isTerminalAlternateScreenActive(sessionId)) {
    return
  }
  ...
  for (const char of normalized) {
    if (char === '\r' || char === '\n') {
      // A submitted line means shell is about to process input; wait for next prompt
      // output before considering the session "prompt-ready" again.
      localPromptReadyBySession.delete(sessionId)
...
function isAICommandBarToggleShortcut(event: KeyboardEvent): boolean {
...
async function explainSelectedTerminalText(selectedText: string): Promise<void> {
  ...
  aiCommandBarMessages.value = [...aiCommandBarMessages.value, displayMessage]
  await sendAIMessagesAndStream([...recentContext, displayMessage], {
    scenarioSystemPrompt: buildAIExplainSelectionSystemPrompt(selection, terminalContext),
  })
}
...
async function splitFocusedPaneAndInheritLocation(direction: 'horizontal' | 'vertical'): Promise<boolean> {
  ...
  const beforeSplit = workspace.getActiveSplitDebugSnapshot()
  ...
  if (!workspace.splitFocusedPane(direction)) {
    ...
    writeTerminalError(
      `${t('terminal.layout.error.splitLimitReached', { max: 6 })} [leaf=${beforeSplit.leafCount}, max=${beforeSplit.maxPanes}, focused=${beforeSplit.focusedPaneId ?? 'none'}]`,
    )
    return false
  }

  const afterSplit = workspace.getActiveSplitDebugSnapshot()
  if (afterSplit.leafCount <= beforeSplit.leafCount) {
    writeTerminalError(
      `Split state unchanged [before=${beforeSplit.leafCount}, after=${afterSplit.leafCount}, focused=${afterSplit.focusedPaneId ?? 'none'}]`,
    )
  }
  ...
  // Local split inheritance prefers an absolute path so the new pane can start
  // in the target cwd directly, without emitting an extra `cd` command.
  ...
  if (normalizedSourceCwd) {
    sessionPathSync.hintCwd(targetSessionId, normalizedSourceCwd)
    clearSplitInheritedCwd(targetSessionId)
    logSplitCwdDebug('split local inherit startup cwd set', {
      sourceSessionId,
      targetSessionId,
      normalizedSourceCwd,
    })
...
async function handleContextMenuAction(
  ...
  if (action === 'split-vertical') {
    await splitFocusedPaneAndInheritLocation('vertical')
    return
  }

  if (action === 'split-horizontal') {
    await splitFocusedPaneAndInheritLocation('horizontal')
    return
  }
...
function renderSessionTranscript(sessionId: string): void {
  ...
  terminal.clear?.()
  const transcript = sessionCache.getTranscript(sessionId)
  logTerminalReplayDebug('render-transcript:start', {
    sessionId,
    transcriptChars: transcript.length,
    hasTranscript: Boolean(transcript),
  })
  if (transcript) {
    // Prevent OSC query responses emitted during transcript replay from being forwarded as shell input.
    suppressReplayTriggeredInputTemporarily()
    terminal.write(transcript)
    terminal.scrollToBottom?.()
    logTerminalReplayDebug('render-transcript:written', {
      sessionId,
      transcriptChars: transcript.length,
      ...buildReplayTranscriptDebugData(transcript),
    })
  }
}
    ...
    removeBridgeDataListener = normalizeCleanup(
      bridge.onData((event) => {
        ...
        if (!firstDataLoggedBySession.has(sessionId)) {
          ...
          logTerminalReplayDebug('stream:first-onData-chunk', {
            sessionId,
            elapsedMs: getElapsedSinceBindStart(sessionId),
            chunkChars: chunk.length,
            transcriptCharsBeforeAppend: sessionCache.getTranscript(sessionId).length,
            chunkPreview: formatShellOutputChunk(chunk).slice(0, 220),
          })
        }
        ...
        appendTriggerOutput(sessionId, chunk)
        updateTerminalAlternateScreenState(sessionId, chunk)
        ...
        const cwdHintResult = isTerminalAlternateScreenActive(sessionId)
          ? { hinted: false, promptDetected: false }
          : updateCwdHintFromTerminalData(sessionId, chunk)
  ...
  if (bridge.onExit) {
    try {
      removeBridgeExitListener = normalizeCleanup(
        ...
        terminalAltScreenActiveBySession.delete(payloadSessionId)
        ...
        clearTriggerRuntimeState(payloadSessionId)
...
async function writeInputToSession(
  ...
  if (!normalizedSessionId || !data) {
    logTerminalReplayDebug('write-input:skip-empty', {
  ...
  const normalizedData = formatShellOutputChunk(data)
  logTerminalReplayDebug('write-input:attempt', {
    sessionId: normalizedSessionId,
    dataSize: data.length,
    data: normalizedData,
    trackHistory: options.trackHistory !== false,
    reportUnavailable: options.reportUnavailable !== false,
  })

  const bridge = getTerminalBridge()
  if (!bridge) {
    logTerminalReplayDebug('write-input:bridge-unavailable', {
      sessionId: normalizedSessionId,
      dataSize: data.length,
      data: normalizedData,
    })
  ...
  logTerminalReplayDebug('write-input:result', {
    sessionId: normalizedSessionId,
    dataSize: data.length,
    data: normalizedData,
    writeOk,
  })
  if (writeOk && data.includes('\u0003')) {
    markTerminalCommandTraceInterruptedByUser(normalizedSessionId)
  }
  if (writeOk && options.trackHistory !== false) {
    collectSessionCommandHistory(normalizedSessionId, data)
    if (data.includes('\r') || data.includes('\n')) {
      armTriggerForSession(normalizedSessionId)
    }
  }
  return writeOk
}
...
function shouldTriggerInlineAICommandRequest(data: string, pendingLine: string): boolean {
  ...
  const sessionId = activeBridgeSessionId
  if (!sessionId || !localPromptReadyBySession.has(sessionId)) {
    return false
  }
  if (isTerminalAlternateScreenActive(sessionId)) {
    return false
  }
  const normalizedPendingLine = stripInputControlSequences(pendingLine).trimStart()
  if (!normalizedPendingLine.startsWith('>')) {
    return false
  }
  ...
  if (terminalImeComposing || Date.now() <= inlineAIImeSuppressUntil) {
    const promptText = normalizedPendingLine.slice(1).trim()
    if (!promptText) {
      return false
    }
  }
...
function parseInlineAIRequestFromPendingLine(pendingLine: string): string | null {
  const normalized = stripInputControlSequences(pendingLine).trimStart()
  if (!normalized.startsWith('>')) {
    return null
  }
...
async function maybeHandleInlineAICommandRequest(sessionId: string, data: string): Promise<boolean> {
  ...
  const promptText = parseInlineAIRequestFromPendingLine(pendingLine)
  if (promptText == null) {
    return false
  }
  if (!promptText) {
...
async function bindSession(sessionId: string): Promise<void> {
  ...
  logTerminalReplayDebug('bind:hydrate-begin', {
    sessionId,
    bindingVersion,
    transcriptCharsBeforeHydrate: sessionCache.getTranscript(sessionId).length,
    hasHydratedPersistedState: sessionCache.hasHydratedPersistedState(sessionId),
  })
  await hydrateLocalSessionFromPersistence(sessionId)
  logTerminalReplayDebug('bind:hydrate-finished', {
    sessionId,
    bindingVersion,
    transcriptCharsAfterHydrate: sessionCache.getTranscript(sessionId).length,
    hasHydratedPersistedState: sessionCache.hasHydratedPersistedState(sessionId),
    cwdHint: sessionPathSync.cwdBySession[sessionId] ?? null,
  })
  if (lifecycleDisposed || bindingVersion !== sessionBindingVersion) {
    return
  }
  ...
  let openedBridgeInThisBind = false
  if (!openedBridgeSessionIds.has(sessionId)) {
    if (sessionCache.hasOpenedSession(sessionId)) {
      ...
      if (!opened) {
        ...
        logTerminalReplayDebug('bind:open-failed', {
          sessionId,
          bindingVersion,
        })
        return
      ...
      logTerminalReplayDebug('bind:open-new-session', {
        sessionId,
        bindingVersion,
        openedWhileRemoteNotReady,
        localShell: openedLocalShell,
      })
  ...
  if (lifecycleDisposed || bindingVersion !== sessionBindingVersion) {
    if (openedBridgeInThisBind) {
      try {
        await bridge.close({ sessionId })
      } catch {
        // Ignore close errors during superseded bind attempts.
      }
      sessionCache.clearOpenedSession(sessionId)
    }
    openedBridgeSessionIds.delete(sessionId)
    sessionPromptBuffer.delete(sessionId)
    resetLocalEchoLine(sessionId)
  ...
  logTerminalReplayDebug('bind:activate-session', {
    sessionId,
    previousSessionId,
    attachedToExistingSession,
    openedInComponent: openedBridgeSessionIds.has(sessionId),
    openedInCache: sessionCache.hasOpenedSession(sessionId),
  })
...
async function bootTerminal(): Promise<void> {
  ...
  logTerminalReplayDebug('boot:start', {
    componentSessionId: props.sessionId,
    activeWorkspaceSessionId: workspace.activeSessionId.value,
    openedInCache: sessionCache.hasOpenedSession(props.sessionId),
  })
  ...
  try {
    ...
    terminal.attachCustomKeyEventHandler?.((event) => {
      const sessionId = activeBridgeSessionId
      if (event.type === 'keydown' && !aiCommandBarVisible.value && isAICommandBarToggleShortcut(event)) {
        event.preventDefault()
        event.stopPropagation()
        dismissTerminalDebugHint()
    ...
    xtermInputSubscription = terminal.onData?.((data) => {
      if (isReplayTriggeredTerminalQueryInput(data)) {
        logTerminalReplayDebug('input-drop:replay-query-response', {
          sessionId: activeBridgeSessionId,
          inputChars: data.length,
          inputPreview: formatShellOutputChunk(data).slice(0, 160),
        })
        return
      }
      let outgoing = data
      if (isReplaySuppressionWindowActive()) {
        const beforeChars = outgoing.length
        outgoing = stripTerminalQueryResponseSequences(outgoing)
        if (outgoing.length !== beforeChars) {
          logTerminalReplayDebug('input-strip:during-replay-suppression', {
            sessionId: activeBridgeSessionId,
            beforeChars,
            afterChars: outgoing.length,
            removedChars: beforeChars - outgoing.length,
          })
        }
      }
      const sessionId = activeBridgeSessionId
      if (sessionId && localPromptReadyBySession.has(sessionId)) {
        // Some full-screen apps (vi/tmux-like flows) may leave terminal query responses
        // in the input stream when returning to shell prompt. Drop these responses so
        // they don't leak into commands as garbled text.
        const beforeChars = outgoing.length
        outgoing = stripTerminalQueryResponseSequences(outgoing)
        if (outgoing.length !== beforeChars) {
          logTerminalReplayDebug('input-strip:prompt-ready', {
            sessionId,
            beforeChars,
            afterChars: outgoing.length,
            removedChars: beforeChars - outgoing.length,
      ...
      void sendInputToBridge(outgoing)
      if (outgoing.includes('\r') || outgoing.includes('\n')) {
        scheduleDelayedCwdRefresh()
      }
    }) ?? null

    attachBridgeListeners()
    const bindNow = shouldBindSessionNow(props.sessionId)
    logTerminalReplayDebug('boot:bind-decision', {
      componentSessionId: props.sessionId,
      bindNow,
      activeWorkspaceSessionId: workspace.activeSessionId.value,
      openedInCache: sessionCache.hasOpenedSession(props.sessionId),
      openedInComponent: openedBridgeSessionIds.has(props.sessionId),
    })
    if (bindNow) {
      ...
      logTerminalReplayDebug('boot:background-bind-scheduled', {
        componentSessionId: props.sessionId,
      })
    }
    ...
    const pasteHandler = (event: ClipboardEvent) => {
      // We fully handle paste here. Stop further listeners (including xterm's own
      // clipboard handler) to avoid duplicate insertion.
      event.preventDefault()
      event.stopPropagation()
    ...
    const keydownHandler = (event: KeyboardEvent) => {
      if (event.isComposing) {
        ...
        if (event.key === 'Enter') {
          inlineAIImeSuppressUntil = Date.now() + INLINE_AI_IME_SUPPRESS_MS
        }
      }
    ...
    mountEl.value.addEventListener('compositionstart', compositionStartHandler, true)
    mountEl.value.addEventListener('compositionend', compositionEndHandler, true)
    const previousHostCleanup = hostInteractionCleanup
    hostInteractionCleanup = () => {
      previousHostCleanup?.()
      mountEl.value?.removeEventListener('compositionstart', compositionStartHandler, true)
      mountEl.value?.removeEventListener('compositionend', compositionEndHandler, true)
    }
  } catch (error) {
    fallbackMode.value = true
...
watch(
  ...
  (sessionIds) => {
    ...
    for (const openedSessionId of Array.from(openedBridgeSessionIds)) {
      ...
      terminalAltScreenActiveBySession.delete(openedSessionId)
      ...
      clearTriggerRuntimeState(openedSessionId)
...
watch(
  aiCommandBarVisible,
  (visible) => {
    removeAICommandBarShortcutListener?.()
    removeAICommandBarShortcutListener = null
    ...
    const keydownHandler = (event: KeyboardEvent) => {
      if (!isAICommandBarToggleShortcut(event)) {
        return
      }
      event.preventDefault()
      event.stopPropagation()
...
onMounted(() => {
  const onAIPlatformsChanged = () => {
    void refreshAIModelTargetsFromSettings()
  }
  window.addEventListener(AI_PLATFORMS_CHANGED_EVENT, onAIPlatformsChanged)
  removeAIPlatformsChangedListener = () => {
    window.removeEventListener(AI_PLATFORMS_CHANGED_EVENT, onAIPlatformsChanged)
  }
  attachPrepareForQuitListener()
  void bootTerminal()
})
...
onBeforeUnmount(() => {
  ...
  detachPrepareForQuitListener()
  removeAICommandBarShortcutListener?.()
  removeAICommandBarShortcutListener = null
  removeAIPlatformsChangedListener?.()
  removeAIPlatformsChangedListener = null
...
<template>
  <section ref="xtermShellEl" class="xterm-shell">
    <div class="xterm-host">
      <div ref="mountEl" class="xterm-host__mount" />
      <div
        v-if="props.dropIndicatorPosition"
        class="xterm-drop-indicator"
        :class="`xterm-drop-indicator--${props.dropIndicatorPosition}`"
      />
    </div>
    ...
    >
      ...
      <div class="ai-command-bar__header">
        <div class="ai-command-bar__header-left">
          ...
          <select
            ...
            :disabled="aiCommandBarLoading || !aiHasConfiguredModel"
            @change="void onAIModelTargetChange()"
          >
            <option v-if="!aiHasConfiguredModel" value="" disabled>
              {{ t('terminal.aiBar.modelSelectEmpty') }}
            </option>
      ...
      <div ref="aiCommandBarContentEl" class="ai-command-bar__content">
        <div class="ai-command-bar__messages">
          ...
          <div
            v-else-if="!aiHasConfiguredModel"
            class="ai-command-bar__msg ai-command-bar__msg--assistant"
          >
            <div class="ai-command-bar__quota-hint">
              <span class="ai-command-bar__quota-hint-text">{{ t('terminal.aiBar.error.notConfiguredHint') }}</span>
              <button type="button" class="ai-command-bar__quota-hint-btn" @click="openAIModelSettings">
                {{ t('terminal.aiBar.error.configureNow') }}
              </button>
            </div>
          </div>
          <p v-else-if="aiCommandBarMessages.length === 0" class="ai-command-bar__placeholder">—</p>
        </div>
      </div>
      <div class="ai-command-bar__input-row">
        <textarea
          ...
          :placeholder="aiHasConfiguredModel ? t('terminal.aiBar.inputPlaceholder') : t('terminal.aiBar.inputDisabledPlaceholder')"
          :disabled="aiCommandBarLoading || !aiHasConfiguredModel"
          rows="2"
...
.ai-command-bar__input:disabled {
  color: #64748b;
