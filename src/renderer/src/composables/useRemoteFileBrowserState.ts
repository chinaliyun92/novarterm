...
export function useRemoteFileBrowserState(params: UseRemoteFileBrowserStateParams) {
  ...
  async function createFile(name: string, content = ''): Promise<string> {
    ...
    await runSftpMutation(async (sessionId) => {
      ...
      const listResult = await window.electronAPI.ssh.sftp.list({
        sessionId,
        remotePath: parentPath,
      })
      const listed = unwrapSSHResult(listResult, t('file.remoteState.error.createFileFailed'))
      if (listed.some((entry) => entry.name === name)) {
        throw new Error(
          t('file.remoteState.error.pathExists', {
            path: remotePath,
          }),
        )
      }
        ...
        for (let index = 0; index < items.length; index += 1) {
          ...
          try {
            ...
            uploadProgress.value = {
              ...uploadProgress.value,
              completed: index + 1,
              percent: toUploadPercent(index + 1, items.length),
            }
          } catch (reason) {
            if (tracker) {
