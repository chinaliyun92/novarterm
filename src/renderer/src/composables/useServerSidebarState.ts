...
function createServerSidebarState() {
  ...
  async function refreshServers(): Promise<void> {
    if (loadPromise) {
      try {
        ...
        // Initial load may have failed; continue and force a fresh fetch below.
      }
    }
  ...
  async function updateServer(serverId: number, input: UpdateServerInput): Promise<ServerRecord> {
    await ensureLoaded()
    const previous = servers.value.find((server) => server.id === serverId) ?? null
    const updated = unwrapResult(await getServerApi().update(serverId, input), t('server.sidebar.error.updateServerFailed'))
    if (previous && isConnectionConfigChange(previous, input)) {
      clearSessionBindingsForServer(serverId)
    }
    await loadServers()
    return updated
  }
  ...
  function isConnectionConfigChange(
  ...
  function clearSessionBindingsForServer(serverId: number): void {
    for (const [sessionId, boundServerId] of Object.entries(sessionServerBindings)) {
      if (boundServerId !== serverId) {
        continue
      }
