...
const CWD_PERSIST_DEBOUNCE_MS = 200
...
function createSessionPathSyncState() {
  ...
  const cwdPersistTimerBySession = new Map<string, ReturnType<typeof setTimeout>>()

  function clearCwdPersistTimer(sessionId: string): void {
    const normalizedSessionId = sessionId.trim()
    if (!normalizedSessionId) {
      return
    }

    const timer = cwdPersistTimerBySession.get(normalizedSessionId)
    if (!timer) {
      return
    }

    clearTimeout(timer)
    cwdPersistTimerBySession.delete(normalizedSessionId)
  }

  async function persistCwdNow(sessionId: string, cwd: string): Promise<void> {
    const normalizedSessionId = sessionId.trim()
    ...
    try {
      await bridge.saveCwd({
        sessionId: normalizedSessionId,
        cwd: normalizedCwd,
      })
    } catch {
      // Ignore persistence failures; cwd hint should still remain usable in memory.
    }
  }

  function schedulePersistCwd(sessionId: string, cwd: string): void {
    const normalizedSessionId = sessionId.trim()
    const normalizedCwd = normalizeCwd(cwd)
    if (!normalizedSessionId || !normalizedCwd) {
      return
    ...
    clearCwdPersistTimer(normalizedSessionId)
    const timer = setTimeout(() => {
      cwdPersistTimerBySession.delete(normalizedSessionId)
      void persistCwdNow(normalizedSessionId, normalizedCwd)
    }, CWD_PERSIST_DEBOUNCE_MS)
    cwdPersistTimerBySession.set(normalizedSessionId, timer)
  }
  ...
  function setCwd(sessionId: string, cwd: string): string | null {
    ...
    if (cwdBySession[normalizedSessionId] === normalizedCwd) {
      return normalizedCwd
    }

    cwdBySession[normalizedSessionId] = normalizedCwd
    schedulePersistCwd(normalizedSessionId, normalizedCwd)
    return normalizedCwd
  }
