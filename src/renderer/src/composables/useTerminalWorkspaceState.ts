...
import { useSessionPathSync } from './useSessionPathSync'
import type {
  CloseConfirmState,
  PaneDropPosition,
  SplitDirection,
...
interface ActiveSplitDebugSnapshot {
  tabId: string | null
  leafCount: number
  maxPanes: number
...
const WORKSPACE_RESTORE_DEBUG_ENABLED = true
...
function summarizeWorkspaceState(state: PersistedWorkspaceState | null): Record<string, unknown> {
  if (!state) {
    return {
      hasState: false,
    }
...
function logWorkspaceRestoreDebug(event: string, detail?: Record<string, unknown>): void {
  if (!WORKSPACE_RESTORE_DEBUG_ENABLED) {
    return
  }
  const timestamp = new Date().toISOString()
  if (detail) {
    console.log(`[workspace-restore][${timestamp}] ${event}`, detail)
    return
  }
  console.log(`[workspace-restore][${timestamp}] ${event}`)
}
...
function summarizeLayoutNode(node: SplitLayoutNode | null): Record<string, unknown> | null {
  if (!node) {
    return null
  }
  ...
  return {
    ...
    splitRatio: node.splitRatio ?? null,
    first: summarizeLayoutNode(node.first),
    second: summarizeLayoutNode(node.second),
  }
}
...
function insertLeafRelativeToTarget(
  node: SplitLayoutNode,
  leaf: SplitLayoutLeafNode,
  targetPaneId: string,
  position: PaneDropPosition,
): { node: SplitLayoutNode; inserted: boolean } {
  const targetLeaf = findLeafById(node, targetPaneId)
  if (!targetLeaf) {
  ...
  const first = insertFirst ? leaf : targetLeaf
  const second = insertFirst ? targetLeaf : leaf
  const branch = createBranchNode(direction, first, second, 0.5)
  const replaced = replaceLeafNodeById(node, targetLeaf.id, branch)
  if (!replaced.replaced) {
    return { node, inserted: false }
  }

  return { node: replaced.node, inserted: true }
}

function moveLeafWithinTree(
  root: SplitLayoutNode,
  sourcePaneId: string,
  targetPaneId: string,
  position: PaneDropPosition,
): { node: SplitLayoutNode; moved: boolean } {
  if (sourcePaneId === targetPaneId) {
    return { node: root, moved: false }
  }
  ...
  const removed = removeLeafNodeById(root, sourceLeaf.id)
  if (!removed.removed || !removed.node) {
    return { node: root, moved: false }
  }

  const inserted = insertLeafRelativeToTarget(removed.node, sourceLeaf, targetPaneId, position)
  if (!inserted.inserted) {
    return { node: root, moved: false }
  }

  return { node: inserted.node, moved: true }
}
...
function restoreSplitState(raw: unknown, sessions: TerminalSession[], preferredSessionId: string | null): SplitState {
  ...
  if (isValidSplitState(raw)) {
    // Migrate old persisted values (for example 2) to the current fixed pane cap.
    // We do not expose max-panes as a user setting, so runtime should always follow MAX_SPLIT_PANES.
    const maxPanes = MAX_SPLIT_PANES
...
function createWorkspaceState() {
  ...
  async function initializeWorkspaceState(): Promise<void> {
    logWorkspaceRestoreDebug('initialize:start', {
      isRestoring: isRestoring.value,
      hasElectronSettingsApi: Boolean(electronSettingsApi),
    })
    ...
    logWorkspaceRestoreDebug('initialize:loaded', {
      settings: summarizeWorkspaceState(settingsState),
      local: summarizeWorkspaceState(localState),
      preferredSource: preferred.source,
      preferredUpdatedAt: preferred.state?.sessions?.[0]?.updatedAt ?? null,
    })
    if (preferred.state) {
      applyPersistedWorkspaceState(preferred.state)
      persistenceMode.value = preferred.source === 'electron' ? 'electron' : 'local'
      logWorkspaceRestoreDebug('initialize:applied-persisted', {
        source: preferred.source,
        tabsAfterApply: tabs.value.length,
        sessionsAfterApply: sessions.value.length,
        activeTabId: activeTabSessionId.value,
        activeSessionId: activeSessionId.value,
      })
    } else {
      ensureWorkspaceReady()
      logWorkspaceRestoreDebug('initialize:created-default-workspace', {
        tabsAfterEnsure: tabs.value.length,
        sessionsAfterEnsure: sessions.value.length,
      })
    }

    isRestoring.value = false
    logWorkspaceRestoreDebug('initialize:complete', {
  ...
  function collectTabSessionIds(tab: TerminalWorkspaceTab): string[] {
    if (!tab.split.root) {
      return []
    }
    return collectLeafNodes(tab.split.root).map((leaf) => leaf.sessionId)
  }

  function clearSessionsPathSync(sessionIds: string[]): void {
    for (const sessionId of sessionIds) {
      sessionPathSync.clearSession(sessionId)
    }
  }
  ...
  function closeTabNow(tabId: string): void {
    ...
    if (closingTab) {
      clearSessionsPathSync(collectTabSessionIds(closingTab))
    }
  ...
  function requestCloseSession(id: string): void {
    ...
    const sessionIds = collectTabSessionIds(tab)
    const hasPending = sessionIds.some((sessionId) => {
      const session = sessions.value.find((item) => item.id === sessionId)
      return session?.hasPendingInput === true
    })

    if (requiresCloseConfirm(hasPending)) {
      closeConfirm.open = true
      closeConfirm.sessionId = tab.id
      return
    }
  ...
  function movePane(sourcePaneId: string, targetPaneId: string, position: PaneDropPosition): boolean {
    const tab = activeTab.value
    if (!tab || !tab.split.root) {
      return false
    }

    const moved = moveLeafWithinTree(tab.split.root, sourcePaneId, targetPaneId, position)
    if (!moved.moved) {
      return false
    }

    tab.split.root = moved.node
    touchTab(tab.id)
    syncActiveSplitView()
    schedulePersistWorkspaceState(0)
    return true
  ...
  function getActiveSplitDebugSnapshot(): ActiveSplitDebugSnapshot {
    const tab = activeTab.value
    if (!tab || !tab.split.root) {
      return {
        tabId: tab?.id ?? null,
        leafCount: 0,
        maxPanes: tab?.split.maxPanes ?? MAX_SPLIT_PANES,
        focusedPaneId: tab?.split.focusedPaneId ?? null,
        activeSessionId: activeSessionId.value,
        tree: summarizeLayoutNode(tab?.split.root ?? null),
      }
    }

    return {
      tabId: tab.id,
      leafCount: collectLeafNodes(tab.split.root).length,
      maxPanes: tab.split.maxPanes,
      focusedPaneId: tab.split.focusedPaneId,
      activeSessionId: activeSessionId.value,
      tree: summarizeLayoutNode(tab.split.root),
    }
  }
  ...
  function splitFocusedPane(
    ...
    const tab = activeTab.value
    console.warn('[split-trace] splitFocusedPane called', {
      direction,
      hasTab: Boolean(tab),
      activeTabId: tab?.id ?? null,
      activeSessionId: activeSessionId.value,
      optionsKind: options?.kind ?? null,
    })
    if (!tab) {
      console.warn('[split-trace] denied: no active tab')
      return false
    }

    ensureTabSplitConsistency(tab, activeSessionId.value)
    if (tab.split.maxPanes !== MAX_SPLIT_PANES) {
      tab.split.maxPanes = MAX_SPLIT_PANES
    }
    tab.split.direction = direction
    ...
    const leaves = collectLeafNodes(tab.split.root)
    console.warn('[split-trace] before split', {
      tabId: tab.id,
      direction,
    ...
    if (leaves.length >= tab.split.maxPanes) {
      console.warn('[split-trace] denied: max panes reached', {
        leaves: leaves.length,
        maxPanes: tab.split.maxPanes,
      })
      syncActiveSplitView()
    ...
    if (!targetLeaf) {
      console.warn('[split-trace] denied: target leaf not found')
      syncActiveSplitView()
      return false
    }
    ...
    if (!replaced.replaced) {
      console.warn('[split-trace] denied: replace leaf failed', {
    ...
    syncActiveSplitView()
    schedulePersistWorkspaceState(0)
    console.warn('[split-trace] split applied', {
      tabId: tab.id,
      direction,
      nextLeaves: collectLeafNodes(tab.split.root).length,
      focusedPaneId: tab.split.focusedPaneId,
      activeSessionId: activeSessionId.value,
    })
    return true
  ...
  function closeFocusedPane(): boolean {
    ...
    clearSessionsPathSync([targetLeaf.sessionId])
